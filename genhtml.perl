...
sub write_dir_page($$$$$$$$$$$$$$$$$)
{
    my ($name, $rel_dir, $base_dir, $title,
        ...
        $total_fn_hit, $total_br_found, $total_br_hit, $overview,
        $testhash, $testfnchash, $testbrhash, $view_type,
        $sort_type) = @_;
    ...
    write_header(*HTML_HANDLE, $view_type, $trunc_dir,
                 $rel_dir, $overall_found, $overall_hit,
                 $total_fn_found, $total_fn_hit, $total_br_found,
                 $total_br_hit, $sort_type);
...
sub process_dir($)
{
    ...
    foreach $filename (grep(/^\Q$abs_dir\E\/[^\/]*$/, keys(%info_data))) {
        ...
        $overview{$base_name} = [$lines_found,
                                 $lines_hit,
                                 $fn_found,
                                 $fn_hit,
                                 $br_found,
    ...
    foreach (@fileview_sortlist) {
        ...
        write_dir_page($fileview_sortname[$_], $rel_dir,
                       $base_dir, $test_title,
                       $trunc_dir, $overall_found,
                       $overall_hit, $total_fn_found,
                       $total_fn_hit, $total_br_found,
                       $total_br_hit, \%overview,
                       {}, {},
        ...
        write_dir_page("-detail".$fileview_sortname[$_],
                       $rel_dir, $base_dir, $test_title,
                       $trunc_dir, $overall_found, $overall_hit,
                       $total_fn_found, $total_fn_hit, $total_br_found,
                       $total_br_hit, \%overview, \%testhash,
                       \%testfnchash, \%testbrhash, 1,
                       $_);
    }

    # Calculate resulting line counts
    return ($overall_found, $overall_hit, $total_fn_found,
...
sub write_function_page($$$$$$$$$$$$$$$$$$)
{
    my ($base_dir, $rel_dir, $trunc_dir, $base_name, $title,
        $lines_found, $lines_hit, $fn_found, $fn_hit, $br_found,
        $br_hit, $sumcount, $funcdata, $sumfnccount, $testfncdata,
        $sumbrcount, $testbrdata, $sort_type) = @_;
    my $pagetitle;
    my $filename;
...
sub process_file($$$)
{
    ...
    # source code view
    if ($no_sourceview) {
        return ($lines_found, $lines_hit, $fn_found,
    ...
    if ($func_coverage) {
        ...
        foreach (@funcview_sortlist) {
            write_function_page($base_dir, $rel_dir, $trunc_dir,
                                $base_name, $test_title, $lines_found,
                                $lines_hit, $fn_found, $fn_hit,
                                $br_found, $br_hit, $sumcount,
                                $funcdata, $sumfnccount, $testfncdata,
                                $sumbrcount, $testbrdata, $_);
        }
    }
...
sub read_info_file($)
{
        ...
        foreach ($line) {
            ...
            /^end_of_record/ && do {
                ...
                if ($filename) {
                    ...
                    set_info_entry($data, $testdata, $sumcount,
                                   $funcdata, $checkdata, $testfncdata,
    ...
    foreach $filename (keys(%result)) {
        ...
        ($testdata, $sumcount, undef, undef, $testfncdata,
         $sumfnccount, $testbrdata, $sumbrcount) = get_info_entry($data);
...
sub add_testfncdata($$)
{
    ...
    foreach $testname (keys(%{$testfncdata1})) {
        if (defined($testfncdata2->{$testname})) {
            ...
            ($fnccount) =
                add_fnccount($testfncdata1->{$testname},
...
sub add_testbrdata($$)
{
    ...
    foreach $testname (keys(%{$testbrdata1})) {
        if (defined($testbrdata2->{$testname})) {
            ...
            # in both data sets: add
            ($brcount) =
                combine_brcount($testbrdata1->{$testname},
...
sub combine_info_entries($$$)
{
    ...
    ($testdata1, $sumcount1, $funcdata1, $checkdata1,
     $testfncdata1, $sumfnccount1, $testbrdata1, $sumbrcount1
    ) = get_info_entry($entry1);
    ($testdata2, $sumcount2, $funcdata2, $checkdata2,
     $testfncdata2, $sumfnccount2, $testbrdata2, $sumbrcount2
    ...
    foreach $testname (keys(%{$testdata1})) {
        if (defined($testdata2->{$testname})) {
            ...
            ($result_testdata{$testname}) =
                add_counts($testdata1->{$testname}, $testdata2->{$testname});
        } else {
        ...
        ($result_sumcount, $lines_found, $lines_hit) =
            add_counts($result_sumcount, $result_testdata{$testname});
    }
    ...
    # Store result
    set_info_entry(\%result, \%result_testdata,
                   $result_sumcount, $result_funcdata,
                   $checkdata1, $result_testfncdata,
                   $result_sumfnccount, $result_testbrdata,
                   $result_sumbrcount, $lines_found,
                   $lines_hit, $fn_found,
                   $fn_hit, $br_found,
                   $br_hit);
...
sub combine_info_files($$)
{
    ...
    foreach $filename (keys(%hash2)) {
        if ($hash1{$filename}) {
            ...
            $hash1{$filename} =
                combine_info_entries($hash1{$filename}, $hash2{$filename},
                                     $filename);
        } else {
...
sub write_png_files()
{
    ...
    if ($dark_mode) {
        $data{"ruby.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                             0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
                             0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
                             0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                             0xca, 0x00, 0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                             0x45, 0x80, 0x1b, 0x18, 0x00, 0x00, 0x00, 0x39,
                             0x4a, 0x74, 0xf4, 0x00, 0x00, 0x00, 0x0a, 0x49,
        ...
        $data{"ruby.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                             ...
                             0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
                             0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                             0xca, 0x00, 0x00, 0x00, 0x07, 0x74, 0x49, 0x4d,
                             0x45, 0x07, 0xd2, 0x07, 0x11, 0x0f, 0x18, 0x10,
                             0x5d, 0x57, 0x34, 0x6e, 0x00, 0x00, 0x00, 0x09,
                             0x70, 0x48, 0x59, 0x73, 0x00, 0x00, 0x0b, 0x12,
                             0x00, 0x00, 0x0b, 0x12, 0x01, 0xd2, 0xdd, 0x7e,
                             0xfc, 0x00, 0x00, 0x00, 0x04, 0x67, 0x41, 0x4d,
                             0x41, 0x00, 0x00, 0xb1, 0x8f, 0x0b, 0xfc, 0x61,
                             0x05, 0x00, 0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                             0x45, 0xff, 0x35, 0x2f, 0x00, 0x00, 0x00, 0xd0,
                             0x33, 0x9a, 0x9d, 0x00, 0x00, 0x00, 0x0a, 0x49,
                             0x44, 0x41, 0x54, 0x78, 0xda, 0x63, 0x60, 0x00,
    ...
    if ($dark_mode) {
        $data{"amber.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                              0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
                              0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
                              0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                              0xca, 0x00, 0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                              0x45, 0x99, 0x86, 0x30, 0x00, 0x00, 0x00, 0x51,
                              0x83, 0x43, 0xd7, 0x00, 0x00, 0x00, 0x0a, 0x49,
                              0x44, 0x41, 0x54, 0x08, 0xd7, 0x63, 0x60, 0x00,
                              0x00, 0x00, 0x02, 0x00, 0x01, 0xe2, 0x21, 0xbc,
                              0x33, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
                              0x44, 0xae, 0x42, 0x60, 0x82
        ...
        $data{"amber.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                              ...
                              0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                              0xca, 0x00, 0x00, 0x00, 0x07, 0x74, 0x49, 0x4d,
                              0x45, 0x07, 0xd2, 0x07, 0x11, 0x0f, 0x28, 0x04,
                              0x98, 0xcb, 0xd6, 0xe0, 0x00, 0x00, 0x00, 0x09,
                              0x70, 0x48, 0x59, 0x73, 0x00, 0x00, 0x0b, 0x12,
                              0x00, 0x00, 0x0b, 0x12, 0x01, 0xd2, 0xdd, 0x7e,
                              0xfc, 0x00, 0x00, 0x00, 0x04, 0x67, 0x41, 0x4d,
                              0x41, 0x00, 0x00, 0xb1, 0x8f, 0x0b, 0xfc, 0x61,
                              0x05, 0x00, 0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                              0x45, 0xff, 0xe0, 0x50, 0x00, 0x00, 0x00, 0xa2,
                              0x7a, 0xda, 0x7e, 0x00, 0x00, 0x00, 0x0a, 0x49,
                              0x44, 0x41, 0x54, 0x78, 0xda, 0x63, 0x60, 0x00,
                              0x00, 0x00, 0x02, 0x00, 0x01, 0xe5, 0x27, 0xde,
                              0xfc, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
                              0x44, 0xae, 0x42, 0x60, 0x82
    ...
    if ($dark_mode) {
        $data{"emerald.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a,
                                0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d,
                                0x49, 0x48, 0x44, 0x52, 0x00, 0x00,
                                0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
                                0x01, 0x03, 0x00, 0x00, 0x00, 0x25,
                                0xdb, 0x56, 0xca, 0x00, 0x00, 0x00,
                                0x06, 0x50, 0x4c, 0x54, 0x45, 0x00,
                                0x66, 0x00, 0x0a, 0x0a, 0x0a, 0xa4,
                                0xb8, 0xbf, 0x60, 0x00, 0x00, 0x00,
                                0x0a, 0x49, 0x44, 0x41, 0x54, 0x08,
                                0xd7, 0x63, 0x60, 0x00, 0x00, 0x00,
                                0x02, 0x00, 0x01, 0xe2, 0x21, 0xbc,
                                0x33, 0x00, 0x00, 0x00, 0x00, 0x49,
        ...
        $data{"emerald.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a,
                                ...
                                0x01, 0x03, 0x00, 0x00, 0x00, 0x25,
                                0xdb, 0x56, 0xca, 0x00, 0x00, 0x00,
                                0x07, 0x74, 0x49, 0x4d, 0x45, 0x07,
                                0xd2, 0x07, 0x11, 0x0f, 0x22, 0x2b,
                                0xc9, 0xf5, 0x03, 0x33, 0x00, 0x00,
                                0x00, 0x09, 0x70, 0x48, 0x59, 0x73,
                                0x00, 0x00, 0x0b, 0x12, 0x00, 0x00,
                                0x0b, 0x12, 0x01, 0xd2, 0xdd, 0x7e,
                                0xfc, 0x00, 0x00, 0x00, 0x04, 0x67,
                                0x41, 0x4d, 0x41, 0x00, 0x00, 0xb1,
                                0x8f, 0x0b, 0xfc, 0x61, 0x05, 0x00,
                                0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                                0x45, 0x1b, 0xea, 0x59, 0x0a, 0x0a,
                                0x0a, 0x0f, 0xba, 0x50, 0x83, 0x00,
                                0x00, 0x00, 0x0a, 0x49, 0x44, 0x41,
                                0x54, 0x78, 0xda, 0x63, 0x60, 0x00,
                                0x00, 0x00, 0x02, 0x00, 0x01, 0xe5,
                                0x27, 0xde, 0xfc, 0x00, 0x00, 0x00,
                                0x00, 0x49, 0x45, 0x4e, 0x44, 0xae,
                                0x42, 0x60, 0x82
        ];
    }
    if ($dark_mode) {
        $data{"snow.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                             0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
                             0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
                             0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                             0xca, 0x00, 0x00, 0x00, 0x06, 0x50, 0x4c, 0x54,
                             0x45, 0xdd, 0xdd, 0xdd, 0x00, 0x00, 0x00, 0xae,
                             0x9c, 0x6c, 0x92, 0x00, 0x00, 0x00, 0x0a, 0x49,
                             0x44, 0x41, 0x54, 0x08, 0xd7, 0x63, 0x60, 0x00,
                             0x00, 0x00, 0x02, 0x00, 0x01, 0xe2, 0x21, 0xbc,
                             0x33, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
                             0x44, 0xae, 0x42, 0x60, 0x82
        ...
        $data{"snow.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                             ...
                             0x01, 0x03, 0x00, 0x00, 0x00, 0x25, 0xdb, 0x56,
                             0xca, 0x00, 0x00, 0x00, 0x07, 0x74, 0x49, 0x4d,
                             0x45, 0x07, 0xd2, 0x07, 0x11, 0x0f, 0x1e, 0x1d,
                             0x75, 0xbc, 0xef, 0x55, 0x00, 0x00, 0x00, 0x09,
                             0x70, 0x48, 0x59, 0x73, 0x00, 0x00, 0x0b, 0x12,
                             0x00, 0x00, 0x0b, 0x12, 0x01, 0xd2, 0xdd, 0x7e,
                             0xfc, 0x00, 0x00, 0x00, 0x04, 0x67, 0x41, 0x4d,
                             0x41, 0x00, 0x00, 0xb1, 0x8f, 0x0b, 0xfc, 0x61,
                             ...
                             0xc2, 0xd3, 0x7e, 0x00, 0x00, 0x00, 0x0a, 0x49,
                             0x44, 0x41, 0x54, 0x78, 0xda, 0x63, 0x60, 0x00,
                             0x00, 0x00, 0x02, 0x00, 0x01, 0xe5, 0x27, 0xde,
                             0xfc, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
                             0x44, 0xae, 0x42, 0x60, 0x82
    ...
    if ($dark_mode) {
        $data{"updown.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                               0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
                               0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x0e,
                               0x08, 0x06, 0x00, 0x00, 0x00, 0x16, 0xa3, 0x8d,
                               0xab, 0x00, 0x00, 0x00, 0x43, 0x49, 0x44, 0x41,
                               0x54, 0x28, 0xcf, 0x63, 0x60, 0x40, 0x03, 0x77,
                               0xef, 0xde, 0xfd, 0x7f, 0xf7, 0xee, 0xdd, 0xff,
                               0xe8, 0xe2, 0x8c, 0xe8, 0x8a, 0x90, 0xf9, 0xca,
                               0xca, 0xca, 0x8c, 0x18, 0x0a, 0xb1, 0x99, 0x82,
                               0xac, 0x98, 0x11, 0x9f, 0x22, 0x64, 0xc5, 0x8c,
                               0x84, 0x14, 0xc1, 0x00, 0x13, 0xc3, 0x80, 0x01,
                               0xea, 0xbb, 0x91, 0xf8, 0xe0, 0x21, 0x29, 0xc0,
                               0x89, 0x89, 0x42, 0x06, 0x62, 0x13, 0x05, 0x00,
                               0xe1, 0xd3, 0x2d, 0x91, 0x93, 0x15, 0xa4, 0xb2,
        ...
        $data{"updown.png"} = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
                               ...
                               0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x0e,
                               0x08, 0x06, 0x00, 0x00, 0x00, 0x16, 0xa3, 0x8d,
                               0xab, 0x00, 0x00, 0x00, 0x3c, 0x49, 0x44, 0x41,
                               0x54, 0x28, 0xcf, 0x63, 0x60, 0x40, 0x03, 0xff,
                               0xa1, 0x00, 0x5d, 0x9c, 0x11, 0x5d, 0x11, 0x8a,
                               0x24, 0x23, 0x23, 0x23, 0x86, 0x42, 0x6c, 0xa6,
                               0x20, 0x2b, 0x66, 0xc4, 0xa7, 0x08, 0x59, 0x31,
                               0x23, 0x21, 0x45, 0x30, 0xc0, 0xc4, 0x30, 0x60,
                               0x80, 0xfa, 0x6e, 0x24, 0x3e, 0x78, 0x48, 0x0a,
                               0x70, 0x62, 0xa2, 0x90, 0x81, 0xd8, 0x44, 0x01,
                               0x00, 0xe9, 0x5c, 0x2f, 0xf5, 0xe2, 0x9d, 0x0f,
...
sub write_header_line(*@)
{
    ...
    foreach $entry (@content) {
        ...
        write_html($handle, "            <td$width$class$colspan>$text</td>\n");
    }
    write_html($handle, "          </tr>\n");
}
    ...
    foreach $line (@lines) {
        ...
        foreach $branch (@$line) {
            ...
            if ($taken eq '-') {
                ...
                $class = "branchCov";
                $text  = " + ";
                $title = "Branch $br_num was taken $taken time";
                $title .= "s" if ($taken > 1);
            }
            $current .= "[" if ($open);
...
sub write_source_line(*$$$$$)
{
    ...
    if ($br_coverage) {
        ...
        foreach (@br_html) {
            write_html($handle, "<span class=\"lineNum\">         </span>$_\n");
        }
    }
    # *************************************************************
...
sub write_html_epilog(*$;$)
{
    ...
    my $f = defined($main::footer) ? $footer :
...
sub write_header(*$$$$$$$$$$)
{
    ...
    if ($type == $HDR_DIR) {
        ...
        $view     = "<a href=\"$base_dir".
            "index.$html_ext\">$overview_title</a> - $esc_trunc_name";
        ...
        if ($frames) {
            ...
            # links
            $view =
                "<a href=\"$base_dir"."index.$html_ext\" ".
            ...
            $view =
                "<a href=\"$base_dir"."index.$html_ext\">".
                "$overview_title</a> - <a href=\"index.$html_ext\">".
                "$esc_dir_name</a> - $esc_base_name";
        }
        ...
        $base_dir = "";
        $view     = "<a href=\"$base_dir".
            "index.$html_ext\">$overview_title</a> - test case descriptions";
    }
    ...
    if (%test_description && ($type != $HDR_TESTDESC)) {
        if ($frames && ($type == $HDR_SOURCE || $type == $HDR_FUNC)) {
            ...
            $test .= " ( <span style=\"font-size:80%;\"><a href=\"$base_dir".
    ...
    # Left row
    push(@row_left,
         [["10%", "headerItem", "Current view:"], ["35%", "headerValue", $view]
         ]);
    push(@row_left,
         [[undef, "headerItem", "Test:"], [undef, "headerValue", $test]]);
    push(@row_left,
         [[undef, "headerItem", "Date:"], [undef, "headerValue", $date]]);
...
sub get_sort_code($$$)
{
    ...
    return ' <span class="tableHeadSort">'.
...
sub write_file_table(*$$$$$$$)
{
    ...
    if (($base_dir ne "") && $show_details) {
        ...
        $line_code = get_line_code($detailed ? $HEAD_DETAIL_SHOWN :
                                   ...
                                   $base_dir);
        $func_code = get_func_code(
                          $detailed ? $HEAD_DETAIL_HIDDEN :
                                                   $HEAD_NO_DETAIL, "Functions",
                          $sort && $sort_type != $SORT_FUNC, $base_dir);
        $br_code = get_br_code($detailed ? $HEAD_DETAIL_HIDDEN :
                                                   $HEAD_NO_DETAIL, "Branches",
                               $sort && $sort_type != $SORT_BRANCH, $base_dir);
    } else {
        $file_code = get_file_code($HEAD_NO_DETAIL,
                                   $fileview ? "Filename" : "Directory",
                                   $sort && $sort_type != $SORT_FILE,
                                   $base_dir);
        $line_code = get_line_code($HEAD_NO_DETAIL, $sort_type,
                                  "Line Coverage",
                                  $sort && $sort_type != $SORT_LINE, $base_dir);
...
sub write_source($$$$$$$)
{
    ...
    if (!open(SOURCE_HANDLE, "<", $source_filename)) {
        ...
        for ($line_number = 1; $line_number <= $last_line; $line_number++) {
            push(@file, "/* EOF */");
        }
    } else {
        @file = <SOURCE_HANDLE>;
    ...
    foreach (@file) {
        ...
        push(@result,
             write_source_line(
                         HTML_HANDLE, $line_number,
                         $_, $count_data{$line_number},
                         $converted->{$line_number}, $sumbrcount->{$line_number}
             ));
    }
...
sub funcview_get_sorted($$$)
{
    ...
    return
        sort({ $sumfncdata->{$b} == $sumfncdata->{$a} ?
                $a cmp $b :
                $sumfncdata->{$a} <=> $sumfncdata->{$b} } keys(%{$sumfncdata}));
}
...
sub apply_baseline($$)
{
    ...
    foreach $filename (keys(%data_hash)) {
        ...
        ($data_testdata, undef, $data_funcdata, $data_checkdata,
         $data_testfncdata, undef, $data_testbrdata) = get_info_entry($data);
        (undef, $base_count, undef, $base_checkdata,
         undef, $base_sumfnccount, undef, $base_sumbrcount
        ) = get_info_entry($base);
        ...
        foreach $testname (keys(%{$data_testdata})) {
            ...
            ($data_testfnccount) =
                subtract_fnccounts($data_testfnccount, $base_sumfnccount);
            ($data_testbrcount) =
                combine_brcount($data_testbrcount, $base_sumbrcount, $BR_SUB);
            ...
            ($sumbrcount, $br_found, $br_hit) =
                combine_brcount($sumbrcount, $data_testbrcount, $BR_ADD);
        }

        # Write back resulting entry
        set_info_entry($data, $data_testdata, $sumcount,
                       $data_funcdata, $data_checkdata, $data_testfncdata,
                       $sumfnccount, $data_testbrdata, $sumbrcount,
                       $found, $hit, $fn_found,
                       $fn_hit, $br_found, $br_hit);
...
sub remove_unused_descriptions()
{
    ...
    if ($after < $before) {
        info("Removed ".
             ($before - $after)." unused descriptions, $after remaining.\n");
    }
}
